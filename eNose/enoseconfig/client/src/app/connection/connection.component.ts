...
export class ConnectionComponent implements OnInit {
  ...
  oldvalue!:any
  constructor(private connectionService: ConnectionService, private bredService: BredService) { }
  ngOnInit(): void {
  ...
  _getAllSensor() {
    this.connectionService.getAllSensors()
      .then((sensors) => {
  ...
  _getAllWifi() {
    this.connectionService.getAllWifi()
      .then((resp) => {
        ...
        console.log(this._WifiResponse.available_networks,"kkkk");
        this.chnageTab(1);
      })
      .catch((err) => {
  ...
  _getModelBySensorId() {
    this.connectionService.getAllModelBySensorId(this.selectedSensor.sensorId)
      .then((model) => {
        if (model.length > 0) {
  ...
  _getAllModels() {
    this.connectionService.getAllModels()
      .then((sensors) => {
  ...
  cancelProcess(isCloseModel = false) {
    this.connectionService.calibration_cancel()
      .then((res) => {
  ...
  startDownload() {
    this.connectionService.calibration_start()
      .then((res) => {
  ...
  finished() {
    this.connectionService.calibration_finished(this.calibration_form.value)
      .then(async (res: any) => {
  ...
  changeStatus(sensor: Sensor) {
    ...
    this.connectionService.change_status(sensor.sensorId, status.toString())
      .then((res) => {
        this._getAllSensor()
      })
      .catch((err) => {
  ...
  deleteLast() {
    this.connectionService.deleteModel(this.lastModelId)
      .then((res: any) => {
        Swal.fire({
          title: res.message,
          timer: 2000
